...
const UserCycle = async () => {
  ...
  new_course = await Course.create({
    ...
    creator: new_User._id,
  });

  access_token = await jwt.sign(
    { email: "amaansayyad2001@gmail.com", userId: new_User._id },
    global.access_token,
    {
      algorithm: "HS256",
      expiresIn: "10h",
...
describe("course", () => {
  it("GET  getting a course by its mongo _id", async () => {
    ...
    await supertest(app)
      .get(`/course/${new_course.name}/${new_course._id}/`)
      .set(header)
      .expect(200)
      .then((res) => {
        ...
        expect(JSON.stringify(course.creator)).toBe(
          JSON.stringify(new_User._id)
        );
      });

    // if course sent is not in the database
    await supertest(app)
      .get(`/course/${"course_doesn't_exist"}/${fake_id}/`)
      .set(header)
      .expect(200)
      .then((res) => {
  ...
  it("Bookmark a course", async () => {
    ...
    await supertest(app)
      .post(`/home/${new_course._id}/${new_course.name}`)
      .set(header)
      .send({ _userID: new_User._id })
      .expect(202)
      .then((res) => {
        expect(res.body).toEqual({
          message: "successfully bookmarked/unbookmarked",
        });
      });
  ...
  it("Showbookmark of a user", async () => {
    ...
    await supertest(app)
      .get(`/users/${new_User.name}/${new_User._id}`)
      .set(header)
      .expect(200)
      .then((res) => {
        expect(res.body.course.Bookmark.length).toBe(1);
        expect(JSON.stringify(res.body.course._id)).toEqual(
          JSON.stringify(new_User._id)
        );
