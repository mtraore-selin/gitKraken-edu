...
describe("Homepage", () => {
  it("GET  Fetch all courses on homepage", async () => {
    ...
    await supertest(app)
      ...
      .then((res) => {
        ...
        expect(JSON.stringify(course[0]._id)).toEqual(
          JSON.stringify(new_course._id)
        );
      });
  });
  it("Fetch Courses based on categories", async () => {
    ...
    await supertest(app)
      ...
      .then((res) => {
        ...
        expect(JSON.stringify(course[1]._id)).toEqual(
          JSON.stringify(new_course2._id)
        );
      });
  ...
  it("Fetch courses based on user's preferences", async () => {
    ...
    let new_course2 = await Course.create({
      title: "course_test2",
      category: "React",
      name: "vashu",
      discription: "something2",
      creator: new_User._id,
    });
